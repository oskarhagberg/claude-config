...
## .gitignore — an allowlist, not a denylist

`~/.claude` is not yours alone: Claude Code, its plugins, and tools like
Claude Usage.app write into it constantly, and they add new subdirectories
without warning. `chrome/` (the Claude-in-Chrome native host wrapper), `daemon/`
(which holds `control.key`, a credential), `context-mode/`, `jobs/`, `feedback/`
have all appeared on their own.

A denylist has to be updated for each one, and every miss is committed silently.
So the file inverts:

```gitignore
/*                      # ignore everything

!/.gitignore            # then re-include exactly what we track
!/README.md
!/settings.json
!/skills-manager.sh
!/skills/
...
/cmux/config.local.sh   # per-machine, inside an allowed tree
```

A new app directory is ignored by default. The failure mode flips from "a secret
leaks" to "you notice a file you wanted isn't tracked" — the right way round.

**What earns a line in the allowlist:** commit CONFIGURATION. Never STATE
(sessions, caches, logs, history — it conflicts on every pull between machines),
never CREDENTIALS, and never a file another program generates — committing one
means a `git checkout` silently overwrites what that program wrote, which is how
Claude Usage.app gets broken.

Two gitignore traps worth knowing:

- **There is no trailing-comment syntax.** `daemon/  # holds a key` is one literal
  pattern and matches nothing — the credential stays committable. Comments go on
  their own line.
- **A symlink pointing outside the repo** (e.g. `skills/find-skills` -> `~/.agents`)
  commits as a symlink and dangles on any machine without that tree.
